...
Jooperative +set test blah +map test
Jooperative set test blah+map test
Jooperative set test blah + map test
...
skip loading of jooperative.cfg
===================
*/
...
static void Com_WriteCDKey( const char *filename, const char *ikey ) {
	...
	FS_Printf( f, "\n// generated by Jooperative, do not modify\r\n" );
...
void Com_Init( char *commandLine ) {
	...
	Com_Printf( "%s %s %s\n", JOOP_VERSION, CPUSTRING, __DATE__ );
...
//	Swap_Init ();
  ...
  // get dedicated here for proper hunk megs initialization
	...
	s = va("%s %s %s", JOOP_VERSION, CPUSTRING, __DATE__ );
	...
	if ( !Com_AddStartupCommands() )
	{
		...
		if ( !com_dedicated->integer ) {
			Cbuf_AddText("cinematic Jooperative.RoQ\n");
...
void Com_WriteConfigToFile( const char *filename ) {
	...
	FS_Printf (f, "// generated by Jooperative, do not modify\n");
	Key_WriteBindings (f);
...
void Com_WriteConfiguration( void ) {
	...
	Com_WriteConfigToFile( "jooperative.cfg" );
	...
	if (UI_usesUniqueCDKey() && fs && fs->string[0] != 0) {
		...
		Com_WriteCDKey( "joopdata", cl_cdkey );
	}
#endif
}
